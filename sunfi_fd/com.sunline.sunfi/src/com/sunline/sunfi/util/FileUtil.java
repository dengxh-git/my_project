...
public class FileUtil
{
	 ...
	 * @return String
	 */
	@Bizlet(value = "取得系统运行路径HomePath，WEB-INF/sunline/")
	public static String getHomePath() {
		if(homePath == null) {
			...
			if(strPath != null && (strPath.length() > 0)) {
				...
				homePath=ApplicationContext.getInstance().getWarRealPath();
				if(homePath.endsWith(File.separator) == false) {
					homePath = homePath + File.separator;
				}
				homePath = homePath +"WEB-INF"+File.separator+"sunline";
			}
			if(homePath.endsWith(File.separator) == false) {
				homePath = homePath + File.separator;
